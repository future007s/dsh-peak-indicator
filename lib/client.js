...
//   peak    = Beijing 09:00-12:00 and 14:00-18:00, Monday through Friday
//   offpeak = every other minute, billed at half the peak price
// and, effective 2026-08-23 00:00 Beijing (2026-08-22 16:00 UTC):
//   Beijing Saturdays and Sundays are off-peak around the clock.
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    zh["settings.compact.title"] = "成本与上下文";
    zh["settings.compact.enabled"] = "自动压缩长会话";
    zh["settings.compact.budget"] = "触发预算";
    zh["settings.compact.retain"] = "保留最近";
    zh["settings.compact.hint"] = "推荐 100k：更低预算更省钱，但旧对话会更多依赖摘要。";
    en["settings.compact.title"] = "Cost & context";
    en["settings.compact.enabled"] = "Auto-compact long sessions";
    en["settings.compact.budget"] = "Trigger budget";
    en["settings.compact.retain"] = "Retain recent";
    en["settings.compact.hint"] = "100k is recommended; lower budgets save more but rely more on summaries.";
    ...
    /**
     * Beijing Saturdays and Sundays bill off-peak around the clock, announced
     * effective 2026-08-23 00:00 Beijing — which is 2026-08-22 16:00 UTC.
     */
    var WEEKEND_OFF_PEAK_FROM_MS = Date.UTC(2026, 7, 22, 16, 0, 0);
    ...
    /**
     * Beijing calendar fields for `now`: minutes since midnight, and day of
     * week (0 = Sunday ... 6 = Saturday). Shifting the instant and reading it
     * in UTC gives the same minute as the previous `% 1440` form and also
     * keeps the date, which the modulo threw away.
     */
    function beijingParts(now) {
      const shifted = new Date(now.getTime() + BEIJING_OFFSET_MINUTES * 60000);
      return {
    ...
    function beijingMinutes(now) {
      return beijingParts(now).minutes;
    }

    /** Whether `now` falls on a Beijing weekend that bills off-peak all day. */
    function isWeekendOffPeak(now) {
      if (now.getTime() < WEEKEND_OFF_PEAK_FROM_MS) return false;
      const day = beijingParts(now).day;
      return day === 0 || day === 6;
    }
    ...
    function currentPeriod(now) {
      ...
      if (isWeekendOffPeak(ref)) return "offpeak";
      const bj = beijingMinutes(ref);
      return PEAK_WINDOWS.some(([startHour, endHour]) => bj >= startHour * 60 && bj < endHour * 60) ? "peak" : "offpeak";
    }

    /**
     * Minutes until the next peak/off-peak transition. Weekend days have no
     * transitions at all, so this walks forward day by day and skips them —
     * which means the answer can now exceed 1440: on a Friday evening the
     * next change really is Monday 09:00, some 60 hours out. Returning the
     * old within-the-day number there would promise a switch that does not
     * come.
     */
    function nextTransitionInMinutes(now) {
      const ref = now ?? new Date();
      const { minutes: bj, day } = beijingParts(ref);
      const weekendRuleActive = ref.getTime() >= WEEKEND_OFF_PEAK_FROM_MS;
      const boundaries = [];
      for (const [startHour, endHour] of PEAK_WINDOWS) boundaries.push(startHour * 60, endHour * 60);
      boundaries.sort((a, b) => a - b);
      for (let ahead = 0; ahead <= 7; ahead++) {
        const weekday = (day + ahead) % 7;
        if (weekendRuleActive && (weekday === 0 || weekday === 6)) continue;
        for (const boundary of boundaries) {
          const delta = ahead * 1440 + boundary - bj;
    ...
    function CompactSettingsRow({ t, scope }) {
      const [value, setValue] = (0, import_react.useState)(() => scope.get());
      (0, import_react.useEffect)(() => scope.watch((next) => setValue(next)), [scope]);
      const ac = value.autoCompact ?? {};
      const update = (patch) => scope.update({ autoCompact: { ...ac, ...patch } }).catch(() => {});
      const budget = ac.contextBudget ?? 100000;
      const retain = ac.retainTokens ?? 15000;
      return (0, import_react.default.createElement)("div", { style: { borderBottom: "1px solid var(--dsw-alias-border-l2)", padding: "16px 0" } },
        (0, import_react.default.createElement)("div", { style: { color: "var(--dsw-alias-label-primary)", fontSize: 14, marginBottom: 10 } }, t("settings.compact.title")),
        (0, import_react.default.createElement)("label", { style: { display: "flex", gap: 8, alignItems: "center", fontSize: 13 } },
          (0, import_react.default.createElement)("input", { type: "checkbox", checked: Boolean(ac.enabled), onChange: (e) => update({ enabled: e.target.checked }) }),
          t("settings.compact.enabled")),
        (0, import_react.default.createElement)("div", { style: { display: "flex", gap: 10, marginTop: 10, alignItems: "center", fontSize: 13 } },
          (0, import_react.default.createElement)("label", null, t("settings.compact.budget")),
          (0, import_react.default.createElement)("select", { value: budget, onChange: (e) => update({ contextBudget: Number(e.target.value) }) },
            [60000, 100000, 150000, 200000].map((n) => (0, import_react.default.createElement)("option", { key: n, value: n }, `${Math.round(n / 1000)}k`))),
          (0, import_react.default.createElement)("label", null, t("settings.compact.retain")),
          (0, import_react.default.createElement)("select", { value: retain, onChange: (e) => update({ retainTokens: Number(e.target.value) }) },
            [8000, 15000, 30000].map((n) => (0, import_react.default.createElement)("option", { key: n, value: n }, `${Math.round(n / 1000)}k`)))),
        (0, import_react.default.createElement)("div", { style: { color: "var(--dsw-alias-label-secondary)", fontSize: 12, marginTop: 8 } }, t("settings.compact.hint")));
    }
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject("settings.plugin.item", () => ctx.slots.register({
        ...
        id: "peak-indicator-compaction",
        key: "peak-indicator",
        order: 40,
        locale: NS,
        inject: () => ({ scope: ctx.settingsScope.bind({ namespace: "peak-indicator" }) })
      }, CompactSettingsRow));
      ctx.slots.inject("settings.general.item", () => ctx.slots.register({
        name: "settings.general.item",
        id: "peak-indicator-compaction-general",
        order: 40,
        locale: NS,
        inject: () => ({ scope: ctx.settingsScope.bind({ namespace: "peak-indicator" }) })
      }, CompactSettingsRow));
