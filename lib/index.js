...
//   from   2026-08-23 00:00 Beijing (2026-08-22 16:00 UTC):
//                      Beijing Saturdays and Sundays are off-peak all day
 ...
 */
const WEEKEND_OFF_PEAK_FROM_MS = Date.UTC(2026, 7, 22, 16, 0, 0);
...
/**
 * Beijing calendar fields for `now`: minutes since midnight, and day of week
 * (0 = Sunday ... 6 = Saturday). Shifting the instant and reading it in UTC
 * keeps the minute arithmetic identical to the previous `% 1440` form while
 * also carrying the date, which the modulo threw away.
 */
function beijingParts(now) {
  const shifted = new Date(now.getTime() + BEIJING_OFFSET_MINUTES * 60_000);
  return {
...
function currentPeriod(now, config) {
  const { minutes: bj, day } = beijingParts(now);
  // Weekend-ness is a fact about the Beijing calendar, not the UTC one: the
  // Beijing weekend runs Friday 16:00 UTC through Sunday 16:00 UTC. Gated on
  // the announcement's effective instant so that replaying an older session
  // log still prices its weekends under the rules that applied back then.
  const weekend =
    now.getTime() >= WEEKEND_OFF_PEAK_FROM_MS && (day === 0 || day === 6);
  const peak =
    !weekend &&
    config.peakWindows.some(([startHour, endHour]) => bj >= startHour * 60 && bj < endHour * 60);
...
// dependency: older/current profiles may not register that service during
// boot, which would leave this otherwise usable plugin stuck in `pending`.
var inject = ["sessionProjections", "tokenMeter", "loader", "settings"];
const SETTINGS_NS = settingsNamespace("peak-indicator");
...
function configureCompaction(ctx, value) {
  ...
  if (entry === void 0) {
    ctx.logger?.warn("peak-indicator: auto-compact requested, but compaction-basic is not present in this profile");
    return;
  }
  return ctx.loader.update("compaction-basic", {
    disabled: false,
    config: {
...
async function apply(ctx, config) {
  ...
  try {
    await configureCompaction(ctx, { autoCompact: ac });
  } catch (error) {
    ctx.logger?.warn(`peak-indicator: failed to configure compaction-basic: ${error.message ?? error}`);
    return;
  }
  ctx.logger?.info(`peak-indicator: auto-compact enabled — compact at ~${ac.contextBudget} tokens (ratio ${compactionRatioForBudget(ac.contextBudget, ac.referenceWindow).toFixed(3)}), retain ~${ac.retainTokens}`);
