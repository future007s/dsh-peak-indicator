...
//     the DeepSeek 2026-08-17 official price list and the peak/off-peak
//     window in effect at each event's own timestamp (event.time).
...
/** Peak price entry for a model id, or null when unknown. */
function resolveModelPrice(model, config) {
  const m = String(model ?? "").toLowerCase();
  let entry = config.prices[m] ?? MODEL_PRICES[m];
  if (entry === void 0) {
    for (const [key, candidate] of Object.entries(MODEL_PRICES)) {
...
/** Effective input/output/cache-hit prices per 1M tokens for a model at a period. */
function priceFor(model, period, config) {
  const peak = resolveModelPrice(model, config);
...
function createPeakCostProjection(config) {
  return {
    ...
    apply: (state, event) => {
      ...
      const price = priceFor(model, period, config);
