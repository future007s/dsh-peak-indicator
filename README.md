...
token 单价（每百万 tokens，输入/输出/缓存命中），末尾附带**当前会话实际消耗费用**（保留两位小数）：

- ⚡ **高峰 入¥3 出¥9 缓存¥0.1 · 本会话¥0.12**（红色徽标）：北京时间 09:00–12:00、14:00–18:00，按高峰价计费
- 🌙 **闲时 入¥1.5 出¥4.5 缓存¥0.05 · 本会话¥0.12**（绿色徽标）：高峰以外时段，价格为高峰的一半
